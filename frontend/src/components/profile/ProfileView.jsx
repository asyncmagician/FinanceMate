...
import { APP_CONFIG, ORIGINAL_CREATOR } from '../../config/app.config';
...
export default function ProfileView() {
  ...
  const saveEmailPreferences = async () => {
    ...
    try {
      await api.updateEmailPreferences(emailPreferences);
      setSuccessMessage(t('profile.emailPreferencesUpdated'));
      setTimeout(() => setSuccessMessage(''), 3000);
    } catch (err) {
      setErrors({ emailPrefs: err.message || t('profile.emailPreferencesError') });
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="grid gap-6">
        ...
        {/* Email Preferences */}
        <div className="card">
          <h2 className="text-xl font-semibold text-obsidian-text mb-4">{t('profile.emailPreferences')}</h2>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <label className="flex items-start gap-3 cursor-pointer group">
                <input
                  ...
                  className="mt-1 rounded border-obsidian-border text-obsidian-accent focus:ring-obsidian-accent"
                />
                <div>
                  <div className="text-obsidian-text group-hover:text-obsidian-accent transition-colors">
              ...
              <label className="flex items-start gap-3 cursor-pointer group">
                <input
                  type="checkbox"
                  checked={emailPreferences.email_budget_alerts}
                  onChange={(e) => setEmailPreferences(prev => ({ 
                    ...prev, 
                    email_budget_alerts: e.target.checked 
                  }))}
                  className="mt-1 rounded border-obsidian-border text-obsidian-accent focus:ring-obsidian-accent"
                />
                <div>
                  <div className="text-obsidian-text group-hover:text-obsidian-accent transition-colors">
                    {t('profile.budgetAlerts')}
                  </div>
                  <div className="text-sm text-obsidian-text-muted">
                    {t('profile.budgetAlertsDesc')}
                  </div>
                </div>
              </label>
              ...
              <label className="flex items-start gap-3 cursor-pointer group">
                <input
                  type="checkbox"
                  checked={emailPreferences.email_weekly_summary}
                  onChange={(e) => setEmailPreferences(prev => ({ 
                    ...prev, 
                ...
                <div>
                  <div className="text-obsidian-text group-hover:text-obsidian-accent transition-colors">
                    {t('profile.weeklySummaries')}
                  </div>
                  <div className="text-sm text-obsidian-text-muted">
                    {t('profile.weeklySummariesDesc')}
                  </div>
                </div>
              </label>
            </div>
            ...
            {errors.emailPrefs && (
              <p className="text-red-400 text-sm">{errors.emailPrefs}</p>
            )}
            ...
            <div className="pt-3">
              ...
              >
                {savingEmailPrefs ? t('profile.saving') : t('save')}
              </button>
            </div>

            <div className="text-xs text-obsidian-text-muted pt-2 border-t border-obsidian-border">
              <p>{t('profile.emailPreferencesNote')}</p>
            </div>
          </div>
        </div>
        ...
        <div className="card bg-obsidian-bg/50">
          ...
          <div className="space-y-3 text-obsidian-text-muted">
            ...
            <div className="pt-2 space-y-2">
              <p>
                <span className="font-medium">{t('profile.version')}:</span> {APP_CONFIG.app.version}
              </p>
              
              {/* Show configurable creator info if enabled */}
              {APP_CONFIG.ui.showCreatorInfo && (
                <>
                  <p>
                    <span className="font-medium">{t('profile.createdBy')}:</span> {APP_CONFIG.creator.name}
                  </p>
                  <div className="flex flex-wrap gap-4 pt-2">
                    {APP_CONFIG.creator.linkedin && (
                      <a 
                        href={APP_CONFIG.creator.linkedin}
                        target="_blank"
                    ...
                    {APP_CONFIG.creator.website && (
                      <a 
                        href={APP_CONFIG.creator.website}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-obsidian-accent hover:underline flex items-center gap-1"
                      >
              ...
              <p className="pt-2">
                ...
                >
                  {APP_CONFIG.repository.url.replace('https://', '')}
                </a>
              </p>
              
              {/* Attribution to original creator */}
              <p className="text-xs text-obsidian-text-muted pt-2">
                Original project by{' '}
                <a 
                  href={ORIGINAL_CREATOR.github}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-obsidian-accent hover:underline"
                >
